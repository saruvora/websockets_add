...
- config.yaml: use this file to configure the server for "Host" and "Port" and configure the client with an input json_data. The logger can also be configured with a name and the type of log level (info, debug, error, critical and warning). 
...
## Input file
- numbers.json: this file is used as the input to the client.py has "operation" (which suggests add), "num1" and "num2". One can either keep one of it in it or repeat it as one wills. Use this file as an input for client.py
...
# Run code 
Simply run the main.py for example after the config file has been configured.
```
python main.py
```
